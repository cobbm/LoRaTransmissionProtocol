...
LRTP::LRTP(uint16_t m_hostAddr)
    : m_hostAddr(m_hostAddr), m_currentLoRaState(LoRaState::IDLE_RECEIVE) {}

std::shared_ptr<LRTPConnection> LRTP::connect(uint16_t destAddr) {
  std::shared_ptr<LRTPConnection> connection = nullptr;
  // check if connection exists
  std::unordered_map<uint16_t, std::shared_ptr<LRTPConnection>>::const_iterator
      connection_iter = m_activeConnections.find(destAddr);
...
int LRTP::begin() {
  // attach callbacks
  LoRa.onReceive(
      std::bind(&LRTP::onLoRaPacketReceived, this, std::placeholders::_1));
...
// callback handlers
void LRTP::onConnect(
    std::function<void(std::shared_ptr<LRTPConnection>)> callback) {
  _onConnect = callback;
}
...
// debug_print_packet(packet);
  ...
  std::unordered_map<uint16_t, std::shared_ptr<LRTPConnection>>::const_iterator
      connection = m_activeConnections.find(packet.src);
...
void LRTP::handleIncomingConnectionPacket(const LRTPPacket &packet) {
  ...
  if (packet.flags.syn && packet.payloadLength == 0) {
    std::shared_ptr<LRTPConnection> newConnection =
        std::make_shared<LRTPConnection>(m_hostAddr, packet.src);
    ...
    lrtp_debugf("%s: Error: invalid packet received - SYN flag not present "
                "or payload included!\n",
                __PRETTY_FUNCTION__);
  }
}
...
void LRTP::loopReceive() {
  ...
  if (m_loraRxBytesWaiting > 0) {
    ...
    LRTPPacket pkt;
    int parseResult =
        LRTP::parsePacket(&pkt, this->m_rxBuffer, m_loraRxBytesWaiting);
    if (parseResult) {
      if (pkt.dest == m_hostAddr) {
        ...
        lrtp_debugf(
            "Packet src: %u, dest: %u, was not addressed to me - ignored "
  ...
  if (m_currentLoRaState == LoRaState::IDLE_RECEIVE) {
    if (m_activeConnections.size() > 0) {
      ...
      static std::unordered_map<
          uint16_t, std::shared_ptr<LRTPConnection>>::const_iterator txTarget =
      ...
      while (txTarget != m_activeConnections.end()) {
        ...
        if (txTarget->second->m_connectionError != LRTPError::NONE) {
          lrtp_debugf(
              "IRQ ERROR: code %u ! (previous errors that occured since the "
              "last check may have been missed)\n",
              txTarget->second->m_connectionError);
...
bool LRTP::beginCAD() {
  ...
  if (channelFree) {
    ...
    lrtp_debugf("beginCAD- rxSignalDetected! Receive rounds %u",
                LORA_SIGNAL_TIMEOUT_ROUNDS);
  }
  return channelFree;
}
...
std::vector<uint8_t> LRTP::preparePacket(const LRTPPacket &packet) {
  ...
  // window size
  uint8_t flagsAndWindow =
      (packFlags(packet.flags) << 0x04) | (packet.ackWindow & 0x0f);
...
void LRTP::sendPacket(const LRTPPacket &packet) {
  ...
  lrtp_debugf(
      "Sending Packet. length: %d. Src: %d, Dest: %u Seq: %u, Ack: %u\n",
      packet.payloadLength, packet.src, packet.dest, packet.seqNum,
      packet.ackNum);
#endif
  setState(LoRaState::TRANSMIT);
...
debugf("CAD %s (%u of ) \n", channelBusy ? "BUSY" : "FREE",
       ...
       */
  lrtp_debugf("CAD %s (%u of %u) ", channelBusy ? "[BUSY]" : "[FREE]",
              m_cadRoundsRemaining, LORA_SIGNAL_TIMEOUT_ROUNDS);
...
void LRTP::setState(LoRaState newState) {
  ...
  Serial.printf("%s: LORA Radio Change State: %s -> %s\n", __PRETTY_FUNCTION__,
                LORAStateToStr(m_currentLoRaState), LORAStateToStr(newState));
#endif
