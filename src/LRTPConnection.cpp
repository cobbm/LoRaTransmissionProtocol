...
LRTPConnection::LRTPConnection(uint16_t source, uint16_t destination)
    : m_srcAddr(source), m_destAddr(destination), m_currentSeqNum(0),
      m_nextAckNum(0), m_seqBase(0), m_windowSize(LRTP_TX_PACKET_BUFFER_SZ),
      m_txDataBuffer(LRTP_MAX_PAYLOAD_SZ * LRTP_TX_PACKET_BUFFER_SZ),
      m_txWindow(m_windowSize), m_connectionState(LRTPConnState::CLOSED) {
...
int LRTPConnection::availableForWrite() {
  ...
  if (m_connectionState == LRTPConnState::CONNECT_SYN_ACK ||
      m_connectionState == LRTPConnState::CONNECTED) {
...
uint16_t LRTPConnection::getRemoteAddr() { return m_destAddr; }

LRTPConnState LRTPConnection::getConnectionState() { return m_connectionState; }

void LRTPConnection::updateTimers(unsigned long t) {
  // check if timers have elapsed
  if (m_timer_packetTimeoutActive &&
      t - m_timer_packetTimeout > LRTP_PACKET_TIMEOUT) {
    onPacketTimeout();
  }
  // handle piggyback timeout
  if (m_timer_piggybackTimeoutActive &&
      t - m_timer_piggybackTimeout > LRTP_PIGGYBACK_TIMEOUT) {
    onPiggybackTimeout();
  }
}
...
bool LRTPConnection::isReadyForTransmit() {
  ...
  bool canTransmitData =
      connectionOpen &&
      ((dataWaitingForTransmit && positionInWindow < m_windowSize) ||
       (positionInWindow < m_txWindow.count()));
...
LRTPPacket *LRTPConnection::prepareNextPacket() {
  ...
  if (bytesWaiting > 0 && m_txWindow.count() < m_windowSize) {
    ...
    if (nextPacket != nullptr) {
      const int packetPayloadSz =
          min(m_txDataBuffer.count(), (size_t)LRTP_MAX_PAYLOAD_SZ);
      if (packetPayloadSz > 0) {
        uint8_t *payloadBuff =
            (uint8_t *)malloc(sizeof(uint8_t) * packetPayloadSz);
...
LRTPPacket *LRTPConnection::getNextTxPacket() {
  ...
  if (nextPacket != nullptr) {
    ...
    if (m_connectionState == LRTPConnState::CLOSE_FIN &&
        relativeSeqNo >= m_txWindow.count()) {
#if LRTP_DEBUG > 0
...
bool LRTPConnection::handleStateConnectSYN(const LRTPPacket &packet) {
  if (packet.flags.syn && packet.flags.ack &&
      packet.ackNum == m_currentSeqNum + 1) {
    m_nextAckNum = packet.seqNum + 1;
...
void LRTPConnection::setConnectionState(LRTPConnState newState) {
  ...
  Serial.printf("%s: Connection change state: %s -> %s\n", __PRETTY_FUNCTION__,
                connStateToStr(m_connectionState), connStateToStr(newState));
#endif
    ...
    if (adjustedAckNum >= m_seqBase && adjustedAckNum <= sendWindowEnd) {
      if (adjustedAckNum < sendWindowEnd) {
        Serial.printf("%s: Entire window not ack'd - Packet ACK: %u, Base: %u, "
                      "Window end: %u \n",
                      __PRETTY_FUNCTION__, packet.ackNum, m_seqBase,
                      sendWindowEnd);
      ...
      Serial.printf(
          "===== m_currentSeqNum = %u ===== (RESEND ENTIRE WINDOW) \n",
          m_seqBase);
      m_currentSeqNum = m_seqBase;
    }
  }
...
#endif
  if (m_connectionState == LRTPConnState::CONNECT_SYN ||
...
void LRTPConnection::incrementSeqNum() { m_currentSeqNum++; }
